...
pub fn run() {
    ...
    // than building it as a wrap, so there is no plugin directory to scan and
    // nothing is discovered: the plugins in `build-aux/android/gstreamer-plugins`
    // are the only elements that exist, and an element outside that list fails
    // at `gst_element_factory_make` rather than at build time.
    //
    // `gst_init()` does not register them. The ndk-build template shipped with
    // the GStreamer binaries implies it does, but nothing in
    // `libgstreamer-1.0.a` references `gst_init_static_plugins` at all, so the
    // call is ours to make — after `gst::init()`, because registration needs an
    // initialized registry. See `doc/android-media-plan.md`.
    #[cfg(target_os = "android")]
    {
