...
/// Windows or Android, so it is given the same stream of ours that the media
/// viewer plays video with. Without it a voice message shows a duration of
/// `00:00` and never plays, because a backend-less `GtkMediaFile` reports no
/// error either.
#[cfg(any(target_os = "macos", target_os = "windows", target_os = "android"))]
fn media_stream_for_file(file: &gio::File) -> gtk::MediaStream {
