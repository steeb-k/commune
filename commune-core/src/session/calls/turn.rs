//! The TURN and STUN servers a homeserver hands out.
//!
//! The application's `turn.rs`: the credentials are fetched once and kept
//! until they go stale, the URIs are sorted so that a relay over UDP is the
//! one a call gets, and they are respelled the way `webrtcbin` wants them.
//! The raw answer is kept too, for an embedder whose WebRTC takes the
//! homeserver's spelling and the credentials beside it.
...
///
/// The homeserver signs a credential that is valid for a fixed time, and a
/// call that outlives it keeps using it — the relay only checks at
/// allocation time. This margin is about not starting a call with a
/// credential that is about to go stale.
// `Duration::from_mins` would say this better and is not stable yet.
#[allow(clippy::duration_suboptimal_units, reason = "from_mins is not stable")]
...
pub struct TurnServer {
    ...
    ///
    /// The credentials are in the URI, so this is the only part of it that
    /// can be logged.
...
///
/// Measured on 23 August 2026: given three TURN servers, `webrtcbin`
/// gathers a relay candidate from **one** of them. Handed the homeserver's
/// list in order it took the first; handed it reversed it took the
/// last-but-one. So the order decides which relay a call gets, and UDP is
/// the one to want — a relay reached over TCP carries every packet of the
/// call through a stream socket, with the head-of-line blocking that
/// implies.
...
/// call when nothing else can and needs credentials to do it; a STUN
/// server only answers "what address did this packet come from", takes no
...
/// The homeserver's answer as it gave it, for an embedder whose WebRTC
/// takes the URIs and the credentials apart.
#[derive(Debug, Clone)]
pub struct RawTurnCredentials {
    /// The server URIs, `stun:` and `turn:` as the server gave them, in the
...
///
/// The homeserver hands out a username and password that are valid for a
/// while and are not tied to any particular call, so they are fetched once
/// and shared.
#[derive(Debug, Default)]
pub struct TurnCredentials {
    /// The servers, already in `webrtcbin`'s spelling.
    servers: IceServers,
    /// The answer as the homeserver gave it.
    raw: Option<RawTurnCredentials>,
...
impl TurnCredentials {
    ...
    #[must_use]
    pub fn servers(&self) -> &IceServers {
        &self.servers
    }

    /// The answer as the homeserver gave it, if it gave one.
    #[must_use]
    pub fn raw(&self) -> Option<&RawTurnCredentials> {
        self.raw.as_ref()
    }
}
...
///
/// A homeserver is not required to run one, and answers `M_NOT_FOUND` or
/// an empty list when it does not. That is not an error: a call between
/// two people who can reach each other directly works without a relay, and
/// the only way to find out is to try.
pub async fn load_turn_credentials(client: &MatrixClient) -> TurnCredentials {
    ...
    let response = match handle.await.expect("task was not aborted") {
        ...
        Err(error) => {
            // Not an error in itself — a homeserver need not run one — but
            // it decides whether anybody behind a NAT can be called at all,
            // so it is worth saying out loud rather than at debug.
            warn!("The homeserver offered no TURN server: {error}");
            return TurnCredentials::default();
        }
    };

    // Sorted, because only the first of them is going to be used, and a
    // call relayed over UDP is the one to have.
    ...
    // The URIs carry no credentials — those are separate fields — so they
    // are safe to log, and worth logging: a `turn_uris` pointing at a LAN
    // address looks exactly like a working one until somebody calls in
    // from outside.
    debug!(
...
///
/// The homeserver speaks RFC 7064 — `stun:host:port` — and `webrtcbin`
/// wants `stun://host:port`. There are no credentials in either: a STUN
/// server is asked one question, "what address did this packet come from",
/// and needs to know nothing about who is asking.
///
/// **This client names no STUN server of its own, and that has not
/// changed.** Pointing every call at somebody else's server to learn our
/// own address tells that third party a call is happening; the
/// homeserver's operator choosing to name one is a different thing
/// entirely, and refusing to use it only meant a client behind a NAT went
/// without a reflexive candidate — which on a network whose TURN server
/// sits inside the same NAT leaves it with no address the other end can
/// reach at all.
///
/// `stuns:` is refused: it is STUN over TLS, and `webrtcbin` has no
/// spelling for it.
fn webrtcbin_stun_uri(uri: &str) -> Option<String> {
    ...
    // A query string is RFC 7064's `?transport=`, which a STUN URI has no
    // use for and `webrtcbin` does not parse.
...
///
/// The homeserver speaks RFC 7065 — `turn:host:port?transport=udp` — with
/// the credentials alongside. `webrtcbin` wants them inside the URI, as
/// `turn://user:password@host:port`. The two are not the same grammar, and
/// the conversion is not just string concatenation: Synapse's username is
/// a timestamp joined to a full user ID, so it carries both `:` and `@` and
/// has to be percent-encoded or the authority is unparsable.
///
/// `turns:` maps to `turns://`. Anything else is refused rather than
/// guessed at.
fn webrtcbin_turn_uri(uri: &str, username: &str, password: &str) -> Option<String> {
    ...
    let scheme = match scheme.to_ascii_lowercase().as_str() {
        "turn" => "turn",
        "turns" => "turns",
        // `stun:` is handled by `webrtcbin_stun_uri`: it takes no
        // credentials.
        _ => return None,
    };
...
///
/// The unreserved set of RFC 3986 is kept and everything else is escaped,
/// which is more than strictly required — the userinfo grammar also admits
/// sub-delimiters — but a password is opaque and there is nothing to gain
/// from leaving any of it bare.
...
mod tests {
    ...
    fn synapse_credentials_survive_the_move_into_the_authority() {
        // Synapse's username is a timestamp joined to a user ID, so it
        // carries the two characters that would otherwise end the authority
        // early.
    ...
    fn a_stun_uri_the_homeserver_names_is_used() {
        ...
        // Over TLS there is nothing to hand `webrtcbin`, and a TURN URI is
