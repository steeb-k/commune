//! The states a call goes through, and what became of one.
...
/// phrased for the person who was in it. This is about any call the room
/// has seen — including one that rang on another device — and it is what
/// the row in the timeline says afterwards.
#[derive(Debug, Default, Hash, Eq, PartialEq, Clone, Copy)]
...
/// Why a call ended.
///
/// This is what the interface says, which is not the same thing as the
/// `reason` of an `m.call.hangup`: several of the spec's reasons are the
/// same sentence to the person reading it, and one of ours — being answered
/// elsewhere — is not a hangup reason at all.
#[derive(Debug, Default, Hash, Eq, PartialEq, Clone, Copy)]
