//! Putting a room inside a space, and taking it back out.
//!
//! The application's `session/room/spaces.rs`, with the same two halves of
//! the relationship and the same opinion about which one counts.
...
use crate::{session::RoomList, spawn_tokio};
...
/// has one.
pub async fn add_room_to_space(room: &Room, space: &Room) -> Result<(), matrix_sdk::Error> {
    ...
    handle
        ...
        .inspect_err(|error| error!("Could not add the room to the space: {error}"))
}
...
/// names it as a child.
pub async fn remove_room_from_space(room: &Room, space: &Room) -> Result<(), matrix_sdk::Error> {
    let matrix_room = room.matrix_room().clone();
    let matrix_space = space.matrix_room().clone();
    let room_id = room.room_id().to_owned();
    let space_id = space.room_id().to_owned();
    ...
    handle
        .await
        .expect("task was not aborted")
        .inspect_err(|error| error!("Could not remove the room from the space: {error}"))
}
...
pub async fn parent_spaces(room: &Room, room_list: &RoomList) -> Vec<Room> {
    let spaces = room_list
        .snapshot()
        .into_iter()
        .filter(|candidate| candidate.category() == RoomCategory::Space)
        .collect::<Vec<_>>();
