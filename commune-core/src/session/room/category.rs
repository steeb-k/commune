//! The category of a room — the application's `room/category.rs` minus the
//! `glib::Enum` derives and the gettext-backed `Display` (section names are
//! rendered by the UIs).
...
impl RoomCategory {
    ...
    /// category.
    #[must_use]
    pub fn can_change_to(self, category: TargetRoomCategory) -> bool {
        match self {
            Self::Invited => {
                matches!(
                    category,
    ...
    #[must_use]
    pub fn to_target_room_category(self) -> Option<TargetRoomCategory> {
...
///
/// The application models this as the `HighlightFlags` bitflags, but only
/// three of the four combinations occur: nothing, bold alone, and both
/// flags together.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub enum RoomHighlight {
    /// The room has no unread messages.
    #[default]
    None,
    /// The room has unread messages.
    Bold,
    /// The room has unread messages that mention or highlight the user.
    Highlight,
}
