...
//!
//! The application's `utils/matrix/ext_traits.rs` minus its one GTK piece
//! (the `GVariant` conversions for `TimelineEventItemId`, which stay with
//! the widgets).

use matrix_sdk_ui::timeline::{
    EventTimelineItem, MembershipChange, Message, MsgLikeKind, TimelineItemContent,
};
use ruma::{
    UserId,
    events::{
...
pub trait EventTimelineItemExt {
