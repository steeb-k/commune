...
async fn restore_sessions_inner() -> Result<Vec<StoredSession>, oo7::Error> {
    ...
    let items = keyring
        .search_items(&HashMap::from([
            (keys::XDG_SCHEMA, config::app_id()),
            (keys::PROFILE, config::profile()),
        ]))
        .await?;
...
impl StoredSession {
    ...
    fn attributes(&self) -> HashMap<&'static str, String> {
        let mut attributes = HashMap::from([
            ...
            (keys::PROFILE, config::profile().to_owned()),
            (keys::XDG_SCHEMA, config::app_id().to_owned()),
        ]);
