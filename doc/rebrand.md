...
* `build-aux/macos/Info.plist.in` — `CFBundleName` and `CFBundleDisplayName` are
  ...
  `.icns` is generated by `make-icns.sh` and is a derived artefact, not another
  copy of the artwork to keep in step.
* `assets/macos-tahoe-flat.svg` and `assets/macos-legacy-bevel.svg` — the Mac
  builds' own icon, and a fourth and fifth place the artwork lives. They are not
  copies of `appicon.svg`: a Mac icon is a square plate the system encloses,
  where the GNOME icon draws its own silhouette, so a change to the mark has to
  be made in all of them by hand. `bundle.sh` picks between the two by
  `--icon-style`; a Devel build still uses `assets/appicon-devel.svg`.
