...
* `assets/macos-legacy-bevel.svg` — the Mac builds' own icon, and a fourth place
  the artwork lives. It is not a copy of `appicon.svg`: a Mac icon is a square
  plate the system encloses, where the GNOME icon draws its own silhouette, so a
  change to the mark has to be made in both by hand. A Devel build still uses
  `assets/appicon-devel.svg`. `assets/macos-tahoe-flat.svg` is a fifth copy but
  nothing refers to it; see `macos.md`.
