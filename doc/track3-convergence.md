...
| 31 Aug 2026 | `klipy.rs` | `Gif::title()`'s fallback for a GIF the API gave no title for was `gettext("GIF")` and became a bare `"GIF"`. It is the fallback body of the event, so it is a sentence, and it goes into the room. The sentence-and-value rule again — it belongs to whichever UI is sending. Not closed with the key fix: it is bound up with the `glib::Boxed` wrappers `Gif` and `SelectedGif` need, which is the rest of this leaf. | Phase 2, leaf 2 |
