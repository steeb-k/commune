...
| Build host | WSL `Ubuntu-24.04`; the emulator and `adb` stay on the Windows side |
| Android SDK/NDK (Linux) | `~/android/sdk`, NDK `27.2.12479018` |
...
| pixiewood | `~/src/gtk-android-builder` |
| Android Studio jar shim | `~/android/fake-studio` (for `Svg2Avd`, see above) |
| GTK checkout | `~/src/gtk` — builds gtk4-demo |
| libadwaita checkout | `~/src/libadwaita` — does **not** build here, see the GLib section |
| S1 Rust spike | `~/src/gtk-android-rust-spike` — the working Rust-to-APK recipe |
...
`wsl -d Ubuntu-24.04 -- bash -lc '…'` mangles `$VAR` and parenthesised paths before bash sees them,
which produces bewildering errors. Write the script to a file and run
`wsl -d Ubuntu-24.04 -- bash -l /mnt/c/…/script.sh` instead. The `.gitattributes` rule keeping
