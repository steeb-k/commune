...
# Two sheets, one drawing. `eyeball-page.py` renders the desktop ledger;
# `eyeball-page.py android` renders the Android one. Each page keeps its marks
# in its own artifact's browser storage, so the sheets cannot see each other.
SHEETS = {
    "desktop": {
        ...
        "title": "Commune Eyeball Run",
        "subtitle": "Commune",
    },
    "android": {
        "ledger": WORKSPACE / "doc" / "eyeball-android.md",
        "output": WORKSPACE / "doc" / "eyeball-android-run.html",
...
SHEET = SHEETS["desktop"]
LEDGER = SHEET["ledger"]
OUTPUT = SHEET["output"]
ARTIFACT_URL = SHEET["artifact_url"]
...
def legacy_map() -> dict[str, str]:
    ...
    text each identifier stood for. Only the desktop sheet ever had positional
    identifiers; any other sheet has nothing to migrate.
    """
    if SHEET is not SHEETS["desktop"]:
        return {}
    try:
...
def render(sections: list[Section], head: str) -> str:
    ...
    return f"""<!-- Published at {ARTIFACT_URL}
     Generated by doc/eyeball-page.py from {ledger_rel} — do not edit
...
<div class="mark"><b>Eyeball Run</b><span>{SHEET["subtitle"]} &middot; {head}</span></div>
<div class="tally">
...
def main() -> int:
    global SHEET, LEDGER, OUTPUT, ARTIFACT_URL

    name = sys.argv[1] if len(sys.argv) > 1 else "desktop"
    if name not in SHEETS:
        print(f"unknown sheet {name!r}; one of: {', '.join(SHEETS)}", file=sys.stderr)
        return 1
    SHEET = SHEETS[name]
    LEDGER = SHEET["ledger"]
    OUTPUT = SHEET["output"]
