# What still needs a pair of eyes, on Android

The Android sheet of `doc/eyeball-tests.md`. That file records what has been
looked at on desktop; nothing in it says anything about a phone, and the merge
of 27 August 2026 — which brought 172 commits of desktop work onto the Android
port and 109 commits of Android work under the desktop features — invalidated
every sighting the port had of its own. **This file is the Android baseline
sweep: every check the desktop ledger carries, re-run on the device, plus the
sections only Android has.** Once it has been through one full pass, only new
work needs both sheets.

Same conventions as the desktop ledger: `[ ]` not yet looked at on Android,
`[x]` looked at and behaves, `[!]` looked at and wrong (with what happened).
Strike an entry here only for what was seen **on the device** — a date inside
an entry's text is a desktop sighting carried over so the runner knows what
right looks like, not an Android result. A subsection that takes something away
opens with a line in italics beginning **Destructive** and sits at the end of
its section, and the run sheet reads that line the same way the desktop one
does.

Two translations apply throughout: where a check says right-click, long-press;
where a check depends on hover, there may be nothing to translate to — mark it
`[!]` with a note saying the control is unreachable rather than skipping it
silently, because unreachable-on-touch is itself a finding.

---

## How to run this

Build and install per `doc/android.md`; the APK comes out of the WSL build
host, and `adb logcat -s Commune` is where the app's own diagnostics land.

The homeserver harness runs on the host exactly as for desktop:
...
`adb reverse` makes `localhost:8008` on the device reach the harness on the
host, so the ledger's instructions — log in as **alice** via _Another
Homeserver_ → `localhost:8008` — read the same on the phone as on desktop.
This route is stated, not yet seen: if it fails, the fallback is the host's
LAN address, and every check that names `localhost` means that address
instead. The passwords and the cast (alice, bob, carol, admin) are as the
desktop ledger's _How to run this_ says.

**Work newest first.** The two sections below are the newest code on this
sheet — the merge's own seams, then the port's features under the merged code
— and the copied desktop sections follow, oldest first, exactly as the desktop
ledger orders them.

---

## The merge of 27 August 2026 — its own seams

Eighteen files carried real conflicts, and their resolutions are the newest
untested code on this sheet. Each check below is one resolution's visible
surface.

* [ ] **The app starts at all.** The merge moved Android's GStreamer static
      plugin registration and the JVM capture out of `run()` into
      `Application::startup`, following desktop's startup-race rework. Launch:
      the window appears, and `adb logcat -s Commune` shows a line ending
      "plugins registered" with a two-digit count, and no line saying "Could
      not reach the Java VM".
* [ ] **An existing session survives the update.** Install the merged APK over
      the previous build: the session unlocks from the keystore-backed secret
      store without asking to log in again. The JVM capture moved; this is
      what breaks if it moved wrong.
* [ ] **The redesigned first page, on a phone.** Login now offers matrix.org
      first with _Another Homeserver_ beneath — desktop work the port has
      never drawn. The radio rows fit the width, and choosing _Another
      Homeserver_ reveals the entry with the URL keyboard — "/" and "." on
      the first rank, no capitalised first letter. The `input-purpose` was
      re-applied to a row the redesign had moved; this is the check that it
      landed.
* [ ] **The composer's rows share their buttons out right.** The port moves
      attach, emoji, sticker and More to their own row above the entry;
      desktop's new voice button is not in that move, so it should sit beside
      the entry with Send. Check the split is exactly that, nothing overlaps,
      and Send was not pushed off the screen — which is the bug the second
      row exists to prevent.
* [ ] **A voice recording that cannot start says so.** Press the microphone.
      Recording is _expected to fail_: `build-aux/android/patch-manifest.sh`
      declares no `RECORD_AUDIO`, and nothing asks for it at runtime — that is
      known follow-up work, not a finding. What needs eyes is the failure
      itself: `ae417095` made a missing microphone name itself even when it
      fails late, and this is that path's first outing on the platform that
      will hit it every time. A toast naming the microphone is a pass; a
      silent dead button or a crash is the `[!]`.
* [ ] **A notification still arrives put together.** Message the device from
      desktop with Commune backgrounded: one notification, in the room's
      conversation, whose tap opens the room. The send path now carries the
      Android and Windows arms side by side; the Android arm's button payload
      order differs from Windows' on purpose.
* [ ] **A picked photo still sends.** The picker hands a `content://` file;
      the preview appears and the send completes. The path-less early bail
      desktop used to have was removed in the resolution — this confirms the
      copy-once detour still runs.
* [ ] **HEIC and AVIF still draw.** Send one of each from desktop (the
      harness's media fixtures, or any phone photo): both render as images,
      not as files. The named-loader fallback survived a twelve-hunk
      conflict.
* [ ] **A video plays and its viewer closes.** Open a video message full
      screen, play it, close it, three times over: no hang. The
      teardown-on-idle that dodges the `GstPlay` deadlock is intact.
* [ ] **Key export and import round-trip.** Settings → export keys to a file,
      then import the file back: both directions go through the temporary-file
      detour and neither errors.

## What only Android has — `doc/android.md`

The port's own features, each seen working before the merge and none seen
since. The S-step that first proved each is named where it helps.

* [ ] **The one-time setup dialog asks once.** A fresh install's first session
      raises the setup dialog (delivery choice, battery lock — S5b's round);
      dismissing it and relaunching does not raise it again.
* [ ] **One delivery mode at a time.** Flipping the background delivery
      setting starts or stops the sync service to match, immediately — the
      settings page write is what acts, not the next restart.
* [ ] **A push wakes a dead Commune.** Force-stop the app, message alice from
      desktop: a notification arrives anyway (UnifiedPush endpoint as pusher),
...
* [ ] **The offline banner tells the truth after a freeze.** Airplane mode on,
      ...
      offline forever.
* [ ] **The back gesture goes back.** In room → room list → app minimised, one
      gesture per step; in the login flow it pops one page per gesture rather
      than closing the app (S10).
* [ ] **The keyboard capitalises sentences.** New message: the first letter
      offers as a capital, and after a full stop the next one does again
      (S10).
* [ ] **Backspace over composed text.** Type with a composing IME (Gboard
      suggestions on), backspace mid-word, keep typing: no doubled or eaten
      letters. Fixed as `74ef6af3` and seen on the Pixel on 27 August 2026 —
      on the pre-merge build, which is why it is unstruck here.
* [ ] **The space bar does not cancel the word** being composed (S9's reset
      bug stays fixed).
* [ ] **Both attachment pickers work** — the photo picker and the document
      picker (S4), including a file with a shortcode-bearing name (S4 found a
      real shortcode bug once).
* [ ] **The status and navigation bars respect the theme** rather than drawing
      light-on-light (the S3 rounds' insets and theming work, unverified since
      the merge).

## Calls — absent on purpose

Calls are WebRTC over GStreamer, which is not cross-built for Android; see
`doc/android.md`. The feature is compiled out, and what needs eyes is that its
absence is tidy rather than broken.

* [ ] **No call button anywhere.** A DM's header, a room's header and menus
      offer no way to start a call.
* [ ] **An incoming `m.call.invite` does not crash.** From desktop, call a room
      the device has open: the timeline may show the invite row or nothing, but
      the app stays up and the room stays usable.
...
* [ ] **Pin from the context menu.** Seen 23 August 2026. Not yet seen: that
      the entry is _absent_ without the power level for `m.room.pinned_events`,
      which needs a room where we are not the moderator.
* [ ] **The menu keeps up.** Seen 23 August 2026 — the same message was
...
* [ ] **The pinned view lists them.** Seen 23 August 2026 and reported as
...
* [ ] **The unpin button on a row works and does not also jump.** Seen
...
* [ ] **A badge appears** for people who are around, green for online. Seen
...
* [ ] **Nothing regressed in logging in.** The greeter's _Log In_ button and
...
* [ ] **A homeserver that cannot send email says so.** Seen 24 August 2026:
...
* [ ] **The new-room dialog offers a _Kind_.** _Room_ is selected; picking
      ...
      _Create Space_.
* [ ] **The Name box is there whatever is being made.** The group titled
...
* [ ] **The space is listed the moment the toast says so.** The other half of
...
* [ ] **The sender is named, not numbered.** Seen on 24 August 2026: three
...
* [ ] **There are no avatars and no images**, by design. Seen on 24 August
...
* [ ] **The line above the list says nobody can see you looking.** Seen on
      24 August 2026.
